...
public class CountTuple extends BaseTuple {
  ...
  public CountTuple(long timestamp, String key, long dominantOpTime, long kafkaAppendTime, long stimulus, long count) {
    super(timestamp, key, dominantOpTime, kafkaAppendTime, stimulus);
    this.count = count;
  }
  ...
  public static class KryoSerializer extends Serializer<CountTuple> implements Serializable {
    ...
    public void write(Kryo kryo, Output output, CountTuple object) {
      ...
      output.writeLong(object.getDominantOpTime());
    ...
    public CountTuple read(Kryo kryo, Input input, Class<CountTuple> type) {
      return new CountTuple(
          input.readLong(), input.readString(), input.readLong(), input.readLong(), input.readLong(), input.readLong());
    }
  }
}
